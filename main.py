...
SERVICES = {
    ...
    "washer": {"name": "Заливка омывайки", "day": 66, "night": 55},
    "tow": {"name": "Перегон на СТО", "day": 254, "night": 210},
}

CYR_TO_LAT = {"А": "A", "В": "B", "Е": "E", "К": "K", "М": "M", "Н": "H", "О": "O", "Р": "P", "С": "C", "Т": "T", "У": "Y", "Х": "X"}
...
def services_keyboard(page: int, selected: dict, delete_mode: bool):
    ...
    for key, svc in chunk:
        ...
        kb.button(text=svc["name"] + (f" ({count})" if count else ""), callback_data=f"svc|{page}|{key}")
    nav = []
    if start > 0:
...
async def update_service_message(call: CallbackQuery, state: FSMContext):
    ...
    if not services:
        ...
        msg_text += f"\nИтого: {total} ₽"
    await call.message.edit_text(msg_text, reply_markup=services_keyboard(data["page"], services, data["delete_mode"]))

@dp.message(F.text.in_({"start", "menu"}))
async def start_cmd(message: Message, state: FSMContext):
    if message.from_user.id != OWNER_ID: return
    await state.clear()
    await message.answer("Панель смены", reply_markup=get_main_menu(bool(get_active_shift())))
...
async def open_shift_handler(message: Message):
    ...
    if get_active_shift():
        ...
        return
    cursor.execute("INSERT INTO shifts (date, start_time) VALUES (?, ?)", (datetime.now().strftime("%Y-%m-%d"), datetime.now().strftime("%H:%M:%S")))
    conn.commit()
...
async def backdate_shift_save(message: Message, state: FSMContext):
    try: dt = datetime.strptime(message.text, "%Y-%m-%d")
    except ValueError: 
        await message.answer("Неверный формат, попробуйте снова (ГГГГ-ММ-ДД)")
        return
    cursor.execute("INSERT INTO shifts (date, start_time) VALUES (?, ?)", (dt.strftime("%Y-%m-%d"), "00:00:00"))
    conn.commit()
    await state.clear()
    await message.answer(f"Смена на {dt.strftime('%Y-%m-%d')} добавлена", reply_markup=get_main_menu(bool(get_active_shift())))
...
async def add_car_start(message: Message, state: FSMContext):
    ...
    await message.answer("Введи номер машины (можно кириллицей, без региона)", reply_markup=ReplyKeyboardRemove())
...
async def add_car_number(message: Message, state: FSMContext):
    ...
    await state.update_data(car_number=normalized, services={}, page=0, delete_mode=False, price_type=price_type)
    await state.set_state(ShiftFSM.edit_services)
    await message.answer(f"🚗 Машина {normalized}\nВыбери услуги:", reply_markup=services_keyboard(0, {}, False))
...
async def save_car(call: CallbackQuery, state: FSMContext):
    ...
    cursor.execute("INSERT INTO cars (shift_id, car_number, total_sum) VALUES (?, ?, 0)", (shift_id, car_number))
    car_id = cursor.lastrowid
    total = 0
    for key, count in services.items():
        if count > 0:
            price = SERVICES[key][price_type]
            cursor.execute("INSERT INTO services (car_id, service_key, service_name, tariff, count, price) VALUES (?, ?, ?, ?, ?, ?)", (car_id, key, SERVICES[key]["name"], price_type, count, price))
            total += price * count
    cursor.execute("UPDATE cars SET total_sum=? WHERE id=?", (total, car_id))
    conn.commit()
    await state.clear()
    await call.message.answer(f"✅ Машина сохранена\n{car_number}\nИтого: {total} ₽", reply_markup=get_main_menu(True))
    await call.answer()

async def close_shift_logic(shift_id: int) -> int:
    cursor.execute("SELECT SUM(price*count) FROM services JOIN cars ON services.car_id=cars.id WHERE cars.shift_id=?", (shift_id,))
    total = cursor.fetchone()[0] or 0
    cursor.execute("UPDATE shifts SET end_time=?, total_sum=?, archived=1 WHERE id=?", (datetime.now().strftime("%H:%M:%S"), total, shift_id))
    conn.commit()
    return total
...
async def close_shift_handler(message: Message):
    ...
    if not shift_id:
        ...
        return
    total = await close_shift_logic(shift_id)
    kb = InlineKeyboardBuilder()
    kb.button("💰 Денежный отчёт", callback_data="report_money")
    kb.button("🔁 Повторки", callback_data="report_repeat")
    kb.adjust(2)
...
async def report_money(call: CallbackQuery):
    shift_id = get_active_shift()
    if not shift_id: await call.answer("Нет активной смены", show_alert=True); return
...
async def cancel(message: Message, state: FSMContext):
    await state.clear()
    await message.answer("Действие отменено", reply_markup=get_main_menu(bool(get_active_shift())))

@dp.message()
async def fallback(message: Message):
    await message.answer("Используй кнопки меню", reply_markup=get_main_menu(bool(get_active_shift())))
...
async def shift_info(message: Message):
    ...
    start_dt = datetime.strptime(start, "%H:%M:%S") if ":" in start else datetime.strptime(start, "%Y-%m-%d %H:%M:%S")
...
async def history_view(call: CallbackQuery):
    ...
    if not cars:
        ...
        for cid, car, s in cars:
            text += f"🚗 {car} — {s} ₽\n"
            cursor.execute("SELECT service_name, count, price FROM services WHERE car_id=?", (cid,))
            for n, c, p in cursor.fetchall():
                text += f"  • {n} ×{c} = {c*p} ₽\n"
    kb = InlineKeyboardBuilder()
...
async def report_repeat(call: CallbackQuery):
    cursor.execute("SELECT car_number, COUNT(*) FROM cars GROUP BY car_number HAVING COUNT(*)>1")
    cars = cursor.fetchall()
    text = "🔁 Повторки:\n"
    if not cars:
        text += "Нет"
